...
<!-- stylesheets -->
<link rel="stylesheet" href="/resources/css/css_staff/product_list_staff.css" />
    ...
    <nav class="nav-panel">
      <ul class="nav-list">
        <?php foreach ($categories as $cat): ?>
          <li class="nav-item <?php if (strtolower($cat['category']) == strtolower($category))
            echo 'selected-category'; ?>">
            <form action="product_list.php" method="post">
              <input type="hidden" name="category" value="<?php echo htmlspecialchars($cat['category']); ?>">
              <input type="submit" value="<?php echo htmlspecialchars($cat['category']); ?>" class="nav-link <?php if (strtolower($cat['category']) == strtolower($category))
                   echo 'selected-category'; ?>">
            </form>
          </li>
        <?php endforeach; ?>
      ...
      <div class="product-card <?php echo $item['quantity'] <= 0 ? 'product-out-of-stock' : ''; ?>">
        ...
        <div class="product-actions">
          ...
          <form action="../helpers/product_handler.php" method="post" class="delete-form">
            ...
            <input type="hidden" name="category" value="<?php echo $category; ?>">
            <input type="button" value="Delete" class="add-to-cart-button delete-btn"
...
<!-- scripts -->
<script src="/resources/js/js_staff/product_list.js"></script>

<!-- template bottom -->
<?php require_once '../components/templates/main-bottom.php'; ?>


<!-- this script is copied to product_list.js -->
<!-- <script>
